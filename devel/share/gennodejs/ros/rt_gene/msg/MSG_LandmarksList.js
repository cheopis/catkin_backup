...
let MSG_Landmarks = require('./MSG_Landmarks.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class MSG_LandmarksList {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.subjects.forEach((val) => {
      bufferOffset = MSG_Landmarks.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new MSG_LandmarksList(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.subjects[i] = MSG_Landmarks.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.subjects.forEach((val) => {
      length += MSG_Landmarks.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new MSG_LandmarksList(null);
    ...
    if (msg.subjects !== undefined) {
      ...
      for (let i = 0; i < resolved.subjects.length; ++i) {
        resolved.subjects[i] = MSG_Landmarks.Resolve(msg.subjects[i]);
      }
    }
    else {
      resolved.subjects = []
...
module.exports = MSG_LandmarksList;
